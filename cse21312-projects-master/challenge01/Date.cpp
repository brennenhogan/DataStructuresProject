...
{
  if (year - check.getYear() < 0) // if the check year is larger than the expiration year (and therefore is later than it) we will return 1. If they are the same, we will do more tests
    return 1; 
  else if (month - check.getMonth() < 0) // if the check month is after, it is expired
    return 1;
  else if (day - check.getDay() <= 0) // if the check date is the same or larger than the expiration date, then it is expired.
    return 1;
  else // if it passes all of the tests, it is not expired
    return 0;
}
